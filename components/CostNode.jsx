...
const CostNode = ({ code, node, level = 0, onCostChange, path }) => {
    ...
    if (level === 0) {
        return (
            <View className="bg-white rounded-3xl mb-4 overflow-hidden shadow-md border border-gray-100">
                {/* Section Header */}
                <TouchableOpacity
                    className="px-5 py-4 flex-row items-center justify-between bg-gradient-to-r from-slate-800 to-slate-900"
                                ...
                                onChangeText={(val) => {
                                    ...
                                    if (val === '' || /^[\d,]*\.?\d*$/.test(val)) {
                                        ...
                                        if (cleanVal === '' || /^\d*\.?\d*$/.test(cleanVal)) {
                                            // Store the raw input value (without commas)
    ...
    return (
        <View>
            {hasChildren ? (
                ...
                // Regular table row for items without children
